...
def tk_linkAnimToCloth(action, *args):
    ...
    if len(failList) > 0:
        ...
        for fail in failList:
            fails += fail + ' '
            print fail
        cClear('tfFeedback', ('Missing objects! Check the Script Editor for details!'), [.3, 0, 0])  
        print '-------------------------'
...
def tkVirtualProductionHelperUI(*args):
    if (cmds.window('win_tkVirtualProductionHelper', exists=1)):
        cmds.deleteUI('win_tkVirtualProductionHelper')
    myWindow = cmds.window('win_tkVirtualProductionHelper', t=('Virtual Production Helper' + ver), s=1)
    ...
    # visibility
    cmds.frameLayout('flBuild', l='BuildSkel', bgc=(colFrameGreen[0], colFrameGreen[1], colFrameGreen[2]), cll=1, cl=0, cc=partial(cShrinkWin, "win_tkVirtualProductionHelper"))
    cmds.rowColumnLayout(nc=4, cw = [(1, 105), (2, 105), (3, 105), (4, 105)])

    cmds.button(l='Build Skeleton', h=bh1, bgc=(colBrown[0], colBrown[1], colBrown[2]), c=partial(tkTglAll, 'FX'))

 


    cmds.showWindow(myWindow)
    cmds.window('win_tkVirtualProductionHelper', e=1, w=300)




tkVirtualProductionHelperUI()
cShrinkWin('win_tkVirtualProductionHelper')
